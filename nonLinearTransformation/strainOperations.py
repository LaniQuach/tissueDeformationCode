...
def display_strain(strain_matrix, outfldr, v_min, v_max, save):
    fig, axs = plt.subplots(2, 2, sharey=True, figsize=[32,20])
    
    # xx dir
    im1 = axs[0][0].imshow(strain_matrix[1:-1,1:-1,0,0], vmin = v_min[0], vmax = v_max[0])
    divider = make_axes_locatable(axs[0][0])
    cax = divider.append_axes('right', size='5%', pad=0.05)
    cbar = fig.colorbar(im1, cax=cax, orientation='vertical');
    cbar.set_label('strain', fontsize = 25)
    ...
    # xy dir
    im2 = axs[0][1].imshow(strain_matrix[1:-1,1:-1,0,1], vmin = v_min[1], vmax = v_max[1])
    divider = make_axes_locatable(axs[0][1])
    cax = divider.append_axes('right', size='5%', pad=0.05)
    cbar1 = fig.colorbar(im2, cax=cax, orientation='vertical')
    cbar1.set_label('strain', fontsize = 25)
    ...
    # yx dir
    im3 = axs[1][0].imshow(strain_matrix[1:-1,1:-1,1,0], vmin = v_min[2], vmax = v_max[2])
    divider = make_axes_locatable(axs[1][0])
    cax = divider.append_axes('right', size='5%', pad=0.05)
    cbar2 = fig.colorbar(im3, cax=cax, orientation='vertical')
    cbar2.set_label('strain', fontsize = 25)
    ...
    # yy dir
    im4 = axs[1][1].imshow(strain_matrix[1:-1,1:-1,1,1], vmin = v_min[3], vmax = v_max[3])
    divider = make_axes_locatable(axs[1][1])
    cax = divider.append_axes('right', size='5%', pad=0.05)
    cbar3 = fig.colorbar(im4, cax=cax, orientation='vertical')
    cbar3.set_label('strain', fontsize = 25)
