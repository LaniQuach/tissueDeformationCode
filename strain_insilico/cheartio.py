...
def read_bfile(path, ordervtu=False):
...
def get_element_type(ien, element=None, bfile=np.array([])):
    if element == None:
...
def write_mesh(fname, pts, elems, element=None):
    ...
    elems, element = get_element_type(elems, element=element)
...
def bfile_to_vtu(mesh_path, out_name, element = None):
    ...
    ien = B[:,1:-1]
    marker = B[:,-1]
    ien, element = get_element_type(ien, element=element)
...
def patches_to_vtu(mesh, ofile, elem = 'triangle'):
    patches = mesh.cells

    for i, p in enumerate(patches):
        io.write_points_cells(ofile + '_patch%i' % (i+1) + '.vtk', mesh.points, {elem: p.data})
...
def create_bfile(mesh, cells, corrs = None):
    """
    Parameters
    ----------
    ...
    for i, p in enumerate(patches):
        ...
        if compute: # TODO save corr in an array an reload it from there
